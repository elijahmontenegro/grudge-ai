...
Grudge can run agent Bash tool calls inside a Docker container instead of on the host. This is a **workspace fence**, not a security boundary against hostile code.
...
This produces `grudge-sandbox:latest` from `containers/sandbox/Dockerfile`. Rebuild any time the Dockerfile changes.
