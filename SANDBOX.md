...
`go/service/sandbox/docker.go` calls `docker run --rm -i -v <dir>:<dir> ... spidey-sandbox:latest sh -c <command>` per Bash tool call. Each working directory is bind-mounted at the same path so commands the agent constructs against host paths work unchanged inside.
...
If you need Spidey-specific tooling (e.g., a different Python version per project), create a derived image and point `go/service/sandbox/docker.go`'s `Image` constant at it. Resist per-thread image selection — it multiplies the build/cache surface and was not worth the complexity when weighed.
