...
//
// Not safe for concurrent use; the exported Index wraps it in a mutex.
type hnsw struct {
	nodes    []hnswNode
...
// insert adds code to the graph and returns its node id. Node ids are the
// index into h.nodes and are stable (the graph is append-only).
func (h *hnsw) insert(code []uint64) int32 {
	...
	for l := min(layer, h.topLayer); l >= 0; l-- {
		found := h.searchLayer(code, ep, h.efConstr, l)
		mMax := h.m
		if l == 0 {
			mMax = h.mMax0
		}
...
// search returns up to k nearest node ids to code, ascending by distance.
// ef is the base-layer beam width (>= k for useful recall).
func (h *hnsw) search(code []uint64, k, ef int) []distNode {
	if h.entry == -1 {
		return nil
	}
	ep := h.entry
	...
	res := h.searchLayer(code, ep, ef, 0)
	if len(res) > k {
		res = res[:k]
	}
	return res
...
// strictly-closer neighbour until none exists (the ef=1 descent step).
func (h *hnsw) greedyClosest(code []uint64, ep int32, layer int) int32 {
	best := ep
	bestDist := Hamming(code, h.nodes[ep].code)
	for {
		improved := false
		for _, nb := range h.nodes[best].conns[layer] {
			if d := Hamming(code, h.nodes[nb].code); d < bestDist {
...
func (h *hnsw) searchLayer(code []uint64, ep int32, ef, layer int) []distNode {
