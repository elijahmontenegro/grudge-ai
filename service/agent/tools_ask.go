...
// AskRequest is sent to the UI when AskUserQuestion is called. The
// full structured args carry through so the UI can render headers,
// options, and multi-select controls. The UI replies with a map of
...
func registerAskUserTool(c *buildCtx) error {
	askUser, err := functiontool.New(
		...
		func(ctx tool.Context, args AskUserArgs) (AskUserResult, error) {
			...
			c.deps.AskCh <- AskRequest{Args: args, RespCh: respCh}
			select {
			case <-ctx.Done():
