...
func init() {
	// rrc.ChunkText (called from InsertMessage) needs a token
	// estimator. Wire the tiktoken-backed default once for the whole
	// storage test run.
	est, err := tiktoken.New()
	if err != nil {
		panic("storage_test: tiktoken.New: " + err.Error())
	}
