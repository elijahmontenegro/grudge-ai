...
func init() { core.RegisterCountProjection("vertex", CountText) }

// CountText is this adapter's counting projection: the text one wire
// message actually contributes to a Vertex request. Mirrors
// genaicodec.ProtoToContent — text, thinking (sent as Thought parts),
// tool calls (name + args), tool results; attachment blocks are never
// sent, so they must not count against the budget.
