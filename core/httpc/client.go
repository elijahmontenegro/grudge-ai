// Package httpc is the adapter construction kit's HTTP transport:
// Client with injected auth, TTFB-bounded streaming, and the typed
// StatusError whose Unwrap maps status codes onto core's canonical
// error sentinels. Public so out-of-tree adapters get the same
// plumbing the built-ins use — the registry's "write your own
// adapter" extension story depends on it.
package httpc
