...
// ProviderFactory constructs a Provider from the config. Adapters
// register themselves via RegisterProvider(name, factory) in init();
// callers invoke NewProvider(cfg) to dispatch by adapter name.
...
func NewProviderFromJSON(raw json.RawMessage) (Provider, error) {
