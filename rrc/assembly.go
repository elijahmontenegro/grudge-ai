...
func (e *Engine) Assemble(ctx context.Context, req AssembleRequest) (AssembleResult, error) {
	...
	e.mu.Lock()
	edges, err := e.OnMessage(ctx, req.Query, req.Corpus)
	if err != nil {
		e.mu.Unlock()
	...
	for {
		...
		for i, m := range llmMsgs {
			msgTokens[i] = EstimateTokens(TextFromBlocks(m.Content))
			total += msgTokens[i]
		}
...
// AssembleTelemetry reports the assembly's per-stage byte/count
// signals — useful for both diagnostic logging and selection
// publishing.
